...
function showPrize() {
  ...
  // initialize keyboard handling for arena movement
  ...
  // Start by showing menu (already visible in HTML)
function showResult(mode) {
  showScreen('result');
  const isGood = correct >= (mode === 'arena' ? 5 : 2);
  document.getElementById('res-emoji').textContent = isGood ? '🏆' : '💪';
...
// Canvas interactions already wired above; expose small helpers
